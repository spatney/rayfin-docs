...
The same check runs at `rayfin dev` startup. It's content-based, not version-based —
upgrading the CLI to a version that ships identical content produces no nudge.
