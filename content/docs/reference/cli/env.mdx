...
`rayfin up` and `rayfin dev` use to keep `.env.local` in sync — exposed as its own command
so you can regenerate it without a full deploy or local-stack restart.
...
| `--output <dir>` | Directory to write `.env.local` into, relative to the project root. Default: project root. |
...
Run this any time you want to refresh `.env.local` without a deploy or local-stack restart
— for example after manually editing `rayfin/.env`:
