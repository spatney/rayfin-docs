...
The CLI prompts interactively for a template and, once a template with a Data service is
selected, a database dialect — unless you skip the prompt with `-t/--template`. See
[Templates](/docs/reference/cli/templates) for choosing a built-in template, scaffolding
from a git URL, or authoring your own.
...
- **Database dialect** — shown when Data is enabled. `mssql` is the only choice unless
  `RAYFIN_FEATURE_FLAGS=postgresql` is set, which also offers `postgresql`.
- **Auth methods** — Fabric SSO, email/password, and magic-link are the supported methods;
  which are offered depends on the services you enabled.
...
prompts and regenerates the configuration. Use this to enable or disable services, switch
the database dialect, or toggle static hosting without hand-editing `rayfin.yml`.
...
| `--auth-methods <list>` | Comma-separated auth methods to enable, e.g. `fabric`. |
