...
| `password.enabled` | boolean | Email/password sign-in. Local development only — deployed Fabric apps don't support it. |
| `fabric.enabled` | boolean | Fabric SSO (Entra ID). The only auth method available in a deployed Fabric app. |
...
| `email.enabled` | boolean | Enables transactional email (verification, password reset). |
| `email.provider` | string | Email provider identifier. |
| `email.senderName` | string | Display name on outgoing email. |
| `email.verificationTokenExpirationHours` | number | Verification link lifetime. |
| `email.passwordResetTokenExpirationMinutes` | number | Password reset link lifetime. |
| `email.smtp.*` | object | SMTP connection settings (`host`, `port`, `senderEmail`, `username`, `password`, `useSsl`, `useStartTls`, `webPort`) — used locally with the Rayfin Local MailDev container. |
| `passwordless.magicLink.enabled` | boolean | Magic-link sign-in. Requires `email.enabled: true`. |
| `passwordless.magicLink.expiryMinutes` | number | Magic link lifetime. |
| `passwordless.smsOtp.enabled` | boolean | SMS one-time-passcode sign-in. |
...
| `dialect` | `mssql` \| `postgresql` | **Required if `enabled` is `true`.** `postgresql` requires the `postgresql` feature flag — see [Environment variables](/docs/reference/config/environment-variables). Deployed Fabric apps only support `mssql`. |

## `services.storage`

| Key | Type | Description |
| --- | --- | --- |
| `enabled` | boolean | Turns the Storage service (Azurite locally, Azure Storage when deployed) on or off. Requires the `storage` feature flag to configure interactively — see [Environment variables](/docs/reference/config/environment-variables). |
...
services:
  auth:
    enabled: true
    fabric:
      enabled: true # Fabric SSO — required for the deployed app
    password:
      enabled: true # email/password — local development only
    allowedRedirectUris:
...
```prompt title="Add PostgreSQL support to a project"
Set RAYFIN_FEATURE_FLAGS=postgresql, then update my rayfin/rayfin.yml so services.data.dialect
is postgresql instead of mssql. Explain what else I need to check before running
`npx rayfin up` again with this change.
```
