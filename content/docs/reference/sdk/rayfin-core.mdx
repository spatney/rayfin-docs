...
With neither option set, `@decimal()` defaults to `DECIMAL(18,2)` on MSSQL and
`NUMERIC(18,2)` on PostgreSQL.
...
| --- | --- | --- |
| `@uuid()` | UUID | `UNIQUEIDENTIFIER` (MSSQL) / `UUID` (PostgreSQL) |
| `@text()` | string | `NVARCHAR(n)` / `VARCHAR(n)`, sized from `max` |
| `@int()` | integer | `INT` / `INTEGER` |
| `@decimal()` | decimal | `DECIMAL` (MSSQL) / `NUMERIC` (PostgreSQL) |
