...
> [Machine-readable docs](/docs/ai/machine-readable-docs) and the
...
`RAYFIN_FEATURE_FLAGS=storage` to expose storage prompts in `rayfin init` and the
`rayfin dev storage *` command group.
...
`rayfin up` or `rayfin dev storage apply`. It does not, by itself, give you a client to
upload or download files — that surface belongs to `@microsoft/rayfin-storage` and is out
