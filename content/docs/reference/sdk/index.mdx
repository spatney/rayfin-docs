...
| Sign users up, sign them in, and manage sessions | [`@microsoft/rayfin-auth`](/docs/reference/sdk/rayfin-auth) — usually via `client.auth` |
...
In practice, most applications install just two packages:
...
- When in doubt about the exact signature for the version you have installed, use
  `rayfin docs get --symbol <name>` or the [MCP server](/docs/ai/mcp-server) rather than
  trusting a cached mental model of the API — see
  [Machine-readable docs](/docs/ai/machine-readable-docs).
