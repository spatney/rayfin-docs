...
| `@decimal()` | decimal / numeric | `max`, `min`, `precision`, `scale` | Defaults to `DECIMAL(18,2)` (MSSQL) / `NUMERIC(18,2)` (PostgreSQL). |
...
other. Omit both to get the dialect default (`DECIMAL(18,2)` on MSSQL, `NUMERIC(18,2)` on
PostgreSQL). Maximum precision is 28, a limit imposed by the Data API Builder runtime.
