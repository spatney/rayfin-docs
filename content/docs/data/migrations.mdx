...
live — locally or on Microsoft Fabric. It is not schema-only: it syncs runtime settings,
...
The commands above apply during local development too — `rayfin up` (and
`rayfin up db apply`) work against your local Docker services the same way they work
against a Fabric deployment. A separate, preview Docker Compose–based workflow is also
available behind a feature flag, with its own schema-apply subcommand:

```bash
export RAYFIN_FEATURE_FLAGS=docker-local-dev
npx rayfin dev            # start the Docker Compose environment
npx rayfin dev db apply   # apply schema changes to it
npx rayfin dev db apply --force
```

Run `rayfin dev db apply` after changing entities under `rayfin/data/` in this preview
workflow, the same way you would run `rayfin up db apply` in the standard flow. Use
`rayfin dev status` to check container health first if the apply fails.
...
(`rayfin up status`, or `rayfin dev status` in the Docker Compose preview flow) before
retrying.

**Enabling `data` without a `dialect`** — `rayfin.yml` requires `dialect: mssql` (or
`postgresql` for local development) whenever `services.data.enabled` is `true`. Omitting
it causes a 400 error at apply time.
