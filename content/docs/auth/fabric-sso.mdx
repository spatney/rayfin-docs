...
> [!WARNING]
> Fabric SSO only works inside the Fabric portal. It cannot be used from `localhost` — use
> [email and password](/docs/auth/email-password) for local development instead. See
> [Auth](/docs/auth) for the full split between modes.
...
Remind me that Fabric SSO only works inside the Fabric portal, so keep a local development
path (email/password) working too, and tell me how to test each one.
```
