...
description: How Rayfin functions ship to Fabric alongside the rest of your app, and how to run them locally against an active deployment.
---
...
## Local development against a deployment

```bash
npx rayfin dev functions apply
```

This starts a local function host **against your active deployment** — so run `rayfin up`
at least once first to establish one. From there, iterate on
`rayfin/functions/src/function_app.ts` locally; see
[Writing a function](/docs/functions/writing-functions#local-development) for the available
flags (`--port`, `--no-debug`, `--inspect-port`) and what the command wires up for you
automatically.
...
After deploying, tell me how to run `npx rayfin dev functions apply` to test functions
locally against this deployment, and how to call one from the frontend afterward.
```
