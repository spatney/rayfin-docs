...
| `PowerBI` | Power BI APIs | Scopes are still to be determined — check with your platform team |
...
When `rayfin dev functions apply` starts the local host, it uses `InteractiveBrowserCredential`
to acquire tokens for all audiences, so connections work the same way locally as they do once
deployed.
