...
- **Fabric SSO** — the popup flow uses your app's bare origin (e.g. `http://localhost:5173`,
  with no path) as the `postMessage` target origin for the handoff code. See
  [Fabric SSO](/docs/auth/fabric-sso) for the full flow.
- **Magic link callbacks** — the `redirectUri` you pass to `sendMagicLink()` should be an
  origin covered by this list. See [Email and password](/docs/auth/email-password).
...
- **Magic link redirect rejected** — confirm the `redirectUri` passed to `sendMagicLink()`
  shares an origin with an entry in `allowedRedirectUris`.
- **Changes not taking effect** — `rayfin.yml` changes require a restart (`rayfin up`, or
  `rayfin dev` in Rayfin Local) before the new redirect URIs are honored.
